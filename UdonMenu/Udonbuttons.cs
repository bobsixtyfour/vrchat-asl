...
public class CreateUdonMenu : MonoBehaviour {
	[MenuItem("ASLWorld/CreateUdonMenu1")]
	static void CreateUdonMenu1(){
		...
		*****************************************/
		...
		menuroot.GetOrAddComponent<UdonBehaviour>().programSource = AssetDatabase.LoadAssetAtPath<AbstractUdonProgramSource>("Assets/Editor/bob-menu/menu.asset");
...
*****************************************/
/*
		for (int languagenum=0; languagenum<AllLessons.Length;languagenum++){ //I only need indiviual gameobjects for these damn videos, so create a loop that creates all of them.
			GameObject langvideocontainer = new GameObject(signlanguagenames[languagenum,0]+" Videos");
			...
			for (int lessonnum = 0; lessonnum < AllLessons[languagenum].Length; lessonnum++){
				GameObject lessonvideocontainer = new GameObject("L"+(lessonnum+1)+" Videos");
				lessonvideocontainer.transform.position = new Vector3(0, 0, 0);
				lessonvideocontainer.transform.SetParent(langvideocontainer.transform, false);
				lessonvideocontainer.layer = layer;
				for (int wordnum = 0; wordnum < AllLessons[languagenum][lessonnum].GetLength(0); wordnum++){
					if(AllLessons[languagenum][lessonnum][wordnum,3]!=""){ //if url is blank, then don't create video.
						...
						videogo.name="L"+(lessonnum+1) +" S"+(wordnum+1)+" Video";
						videogo.layer = layer;
						videogo.transform.SetParent(lessonvideocontainer.transform, false);
...
for (int x = 0; x < 56; x++){
				if (x != 0){
					...
					GameObject buttongo=createbutton2(parent:menubuttons, name:"Button "+(x+1), sizedelta:buttonsize,
					...
					text: "         Buttontxt"+(x+1),txtsizedelta:new Vector2 (750, 100),txtanchoredPosition:new Vector2 (32,0), 
					alignment:TextAnchor.MiddleLeft, nav:no_nav,layer:layer);
					//Debug.Log("GameObject b"+x+" = GameObject.Find(\"/Udon Menu System/Root Canvas/Menu Buttons/Button "+(x+1)+");");

/*
					Button b = buttongo.GetOrAddComponent<Button>();
					b.onClick = new Button.ButtonClickedEvent();*/
						...
						System.Delegate.CreateDelegate(typeof(UnityAction<string>), menuroot.GetOrAddComponent<UdonBehaviour>()//the target of the action
						, "SendCustomEvent") as UnityAction<string>,"B"+(x+1));
