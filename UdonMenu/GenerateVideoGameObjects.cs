...
public class GenerateVideoGameObjects : MonoBehaviour {
	[MenuItem("ASLWorld/GenerateVideoGameObjects")]
...
new string[]{"Cannot read","Idle","No Data Yet.","https://vrsl.withdevon.xyz/cdn/videos/BSL/1/36_sheezy.mp4","2",""},
    ...
    int layer=11;
GameObject videocontainer = GameObject.Find("/Udon Menu System/Video Container"); 
        for (int languagenum=0; languagenum<AllLessons.Length;languagenum++){ //for every language in AllLessons:
            for (int lessonnum = 0; lessonnum < AllLessons[languagenum].Length; lessonnum++){ //for every lesson in each language:
                for (int wordnum = 0; wordnum < AllLessons[languagenum][lessonnum].Length; wordnum++){//for every word in each lesson:
                    //if(AllLessons[languagenum][lessonnum][wordnum][3]!=""){//if url is blank, then don't create video.
                        GameObject videogo = GameObject.CreatePrimitive(PrimitiveType.Quad);
                            ...
                            videogo.layer = layer;
                            videogo.transform.SetParent(videocontainer.transform, false);
                    if(AllLessons[languagenum][lessonnum][wordnum][3]!=""){//if url is blank, then don't create video.
